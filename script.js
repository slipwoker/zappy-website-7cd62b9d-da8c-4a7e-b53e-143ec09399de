document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getWrapper(field) {
      return field.closest('.form-group, .field, .input-wrapper') || field.parentElement;
    }
  ...
  const OBSERVE_SELECTOR = '.animate-on-scroll, [data-animate], section, .card, .feature, .testimonial, .pricing-card';
  ...
  if (animatableElements.length > 0) {
    animatableElements.forEach(function (el, index) {
      ...
      const delay = el.dataset.animateDelay || (index % 4) * 100;
      if (delay) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ShopZone משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '7cd62b9d-da8c-4a7e-b53e-143ec09399de',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '7cd62b9d-da8c-4a7e-b53e-143ec09399de';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '7cd62b9d-da8c-4a7e-b53e-143ec09399de';
